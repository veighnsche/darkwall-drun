...
pub fn draw(f: &mut Frame, app: &mut App, icon_manager: Option<&Arc<Mutex<IconManager>>>) {
    ...
    match mode {
        ...
        AppMode::PostExecution { ref command, ref exit_status, ref copy_feedback } => {
...
fn draw_post_execution(
    ...
    let total_lines = terminal.total_lines();
    let scroll_indicator = if is_at_bottom { "[bottom]" } else { "[scrolled]" };
    ...
    let status = if let Some(msg) = copy_feedback {
        format!(" {} lines {} | {} | y: copy | Enter: dismiss | q: quit", 
    ...
    let status_bar = Paragraph::new(status)
        .style(Style::default().fg(status_color).bg(theme.background));
    f.render_widget(status_bar, chunks[2]);
}
